...
def create_app(controller: IdleShutdownController) -> FastAPI:
    ...
    async def process_endpoint(
        ...
        try:
            ...
            return JSONResponse(status_code=400, content=error_payload(ERROR_INVALID_URL, str(exc)))
        except VideoUnavailableError as exc:
            return JSONResponse(
                status_code=404, content=error_payload(ERROR_VIDEO_UNAVAILABLE, str(exc))
            )
